...
def put_profile_pic(url, profile):
    ...
    try:
        ...
        filename, headers = urlretrieve(
            "%s/resize?w=600&h=600" % url)

        # store profile sized picture (40x40px)
        resize_filename, headers = urlretrieve(
        "%s/resize?w=40&h=40" % url) 
        conn = S3Connection(env['AWS_ACCESS_KEY_ID'], 
        ...
        k = Key(b)
        k.key = md5.new(
            "%sresize" % profile.user.username).hexdigest()
